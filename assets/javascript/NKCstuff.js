...
{//create a game object
  ...
  category:0,//the number from the list above that we chose for this game
  categoryImage:"x",
  ...
  showAnswer: function(yourAnswer)
  {
    ...
    }).done(function(gifSearch)
    {
      ...
      var p = $("<p id='answerDiv'>").text("The answer was: " + game.theAnswer);//create a p for our rating text
      ...
      {
        game.myDivGameArea.append("<div><h4 class='text-center animated infinite bounce' id = 'totalTime'> right answer!!!!!</h4></div>");
      }
      else if(yourAnswer == 2)//no answer
      {
        game.myDivGameArea.append("<div><h4 class='text-center animated infinite bounce' id = 'totalTime'> no answer!!!!!</h4></div>");
      }
    });
  },
  outtaTime: function() 
